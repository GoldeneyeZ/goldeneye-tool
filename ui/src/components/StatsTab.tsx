...
function HealthDot({ name }: { name: string }) {
  ...
  useEffect(() => {
    fetch(apiUrl(`/api/project-health?name=${encodeURIComponent(name)}`))
      .then((r) => r.json())
      .then((d) => {
...
function AdrButton({ project }: { project: string }) {
  ...
  const fetchAdr = useCallback(async () => {
    try {
      const res = await fetch(apiUrl(`/api/adr?project=${encodeURIComponent(project)}`));
...
function CreateIndexModal({ onClose, onCreated }: { onClose: () => void; onCreated: () => void }) {
  ...
  const browse = useCallback(async (path?: string, opts?: { silent?: boolean }) => {
    ...
    try {
      ...
      const res = await fetch(apiUrl(`/api/browse${q}`));
      const data = await res.json();
  ...
  const submit = async (path = currentPath) => {
    ...
    try {
      ...
      const res = await fetch(apiUrl("/api/index"), { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify(body) });
      const data = await res.json();
...
export function IndexProgress({ onDone }: { onDone: () => void }) {
  ...
  useEffect(() => {
    ...
    const poll = setInterval(async () => {
      try {
        const data = await (await fetch(apiUrl("/api/index-status"))).json();
        setJobs(data);
...
export function StatsTab({ onSelectProject }: StatsTabProps) {
  ...
  const deleteProject = useCallback(async (name: string) => {
    ...
    try { await fetch(apiUrl(`/api/project?name=${encodeURIComponent(name)}`), { method: "DELETE" }); refresh(); } catch { /* */ }
  }, [refresh, t.projects]);
