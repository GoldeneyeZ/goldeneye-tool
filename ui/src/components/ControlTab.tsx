...
function LogViewer() {
  ...
  useEffect(() => {
    const poll = setInterval(async () => {
      try {
        const res = await fetch(apiUrl("/api/logs?lines=200"));
        const data = await res.json();
        setLines(data.lines ?? []);
      } catch { /* ignore */ }
    }, 2000);
    /* Initial fetch */
    fetch(apiUrl("/api/logs?lines=200")).then(r => r.json()).then(d => setLines(d.lines ?? [])).catch(() => {});
...
export function ControlTab() {
  ...
  const killProcess = useCallback(async (pid: number) => {
    ...
    try {
      await fetch(apiUrl("/api/process-kill"), {
        method: "POST",
