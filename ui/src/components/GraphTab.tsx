...
function budgetKey(project: string): string {
  return `goldeneye-node-budget:${project}`;
}
...
export function GraphTab({ project }: GraphTabProps) {
  ...
  const [leftWidth, setLeftWidth] = useState(() => loadWidth("goldeneye-left-w", 260));
  const [rightWidth, setRightWidth] = useState(() => loadWidth("goldeneye-right-w", 280));
  ...
  useEffect(() => {
    ...
    let cancelled = false;
    fetch(apiUrl(`/api/repo-info?project=${encodeURIComponent(project)}`))
          ...
          <ResizeHandle
            ...
            onResize={(d) => {
              setRightWidth((w) => {
                ...
                saveWidth("goldeneye-right-w", nw);
                return nw;
              });
            }}
          />
