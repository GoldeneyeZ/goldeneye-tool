    ...
    engine.kind === "gcal"
      ? engine.gcalDiscoveryStrategy === "single-dossier"
        ? [
            ...
            "- Use the six searches as evidence buckets: option parsing/config, model propagation, layout/copy order, analogous tests, docs/manual, and help/locales/platform variants. Each search string must be one exact identifier/literal or pipe-separated alternatives, never prose. Retain and fetch the best two distinct sources from every bucket before considering extras; do not let early search results consume a global quota. Stay within twelve exact source fetches and the 32-call backend guard.",
            "- Fetch candidates with gcal.trySource so project/module pseudo-symbols or other source misses become explicit gaps instead of rejecting the workflow. The workflow return value must stay below 36 KiB. Never return raw source responses or complete files: project each fetched source into compact metadata plus only the relevant excerpt (at most 2,000 characters per source). Return unresolved exact source IDs so a later batch get can retrieve full text when necessary.",
            "- The dossier must report coverage or an explicit gap for every evidence bucket. After a successful workflow, do not call gcal help or status. Use at most three additional GCAL CLI invocations, only for exact missing evidence; batch exact gets where possible. Never invoke gcal workflow a second time. Stop discovery once all buckets support edits.",
          ].join("\n")
