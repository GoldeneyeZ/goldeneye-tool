    ...
    engine.kind === "gcal"
      ? engine.gcalDiscoveryStrategy === "single-dossier"
        ? [
            ...
            "- Use the six searches as evidence buckets: option parsing/config, model propagation, layout/copy order, analogous tests, docs/manual, and help/locales/platform variants. Each search string must be one exact identifier/literal or pipe-separated alternatives, never prose. Give every search a bucket-specific filePattern regex so docs resolve to docs, locales to resource files, tests to tests, and parser/model/layout evidence to their Java areas. Prefer directory-agnostic suffix patterns such as jpackage\\.md$; never guess an exact directory not already supported by evidence.",
            "- Within each bucket, deduplicate by filePath and retain up to four distinct files before considering extras; do not let early results or duplicate symbols consume a global quota. Fetch at most twenty-four exact sources with gcal.trySource so project/module pseudo-symbols or other source misses become explicit gaps instead of rejecting the workflow. Use any remaining backend budget for at most two targeted traces, always through gcal.tryCallers/gcal.tryCallees so a stale trace target cannot reject the workflow.",
