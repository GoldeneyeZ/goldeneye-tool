...
$Fixtures = @(
	@{ Module = "spring-core"; Source = "spring-core/SensitiveAnnotationAgentBenchTests.java";
		...
		Tests = @(
			"org.springframework.core.annotation.SensitiveAnnotationAgentBenchTests.exposesRuntimeDocumentedMetaAnnotatableMarkerContract"
		) },
	@{ Module = "spring-context"; Source = "spring-context/SensitiveDataBinderAgentBenchTests.java";
		...
		Tests = @(
			"org.springframework.validation.SensitiveDataBinderAgentBenchTests.redactsValidatorRejectedRecordComponentWithoutMutatingTarget",
			"org.springframework.validation.SensitiveDataBinderAgentBenchTests.leavesUnmarkedRejectedValueUnchanged",
			"org.springframework.validation.SensitiveDataBinderAgentBenchTests.redactsSubmittedSecretForTypeMismatchAndPreservesErrorMetadata",
			"org.springframework.validation.SensitiveDataBinderAgentBenchTests.redactsDirectFieldAccessAndNestedIndexedPaths",
			"org.springframework.validation.SensitiveDataBinderAgentBenchTests.redactsConstructorBoundRecordComponent",
			"org.springframework.validation.SensitiveDataBinderAgentBenchTests.supportsCustomDetectorForUnmarkedProperty",
			"org.springframework.validation.SensitiveDataBinderAgentBenchTests.supportsCustomRedactorForSensitiveProperty"
		) },
	@{ Module = "spring-web"; Source = "spring-web/SensitiveWebBindingInitializerAgentBenchTests.java";
		...
		Tests = @(
			"org.springframework.web.bind.support.SensitiveWebBindingInitializerAgentBenchTests"
		) }
)
$AllowedPrefixes = @(
	"spring-core/src/",
...
function Assert-AllowedCandidateFiles {
	...
	if ($actual.Count -lt 8 -or $actual.Count -gt 32 -or $disallowed -or $missingRequiredPrefixes) {
		throw "Protocol violation: candidate changes must use the four Spring module policy (8-32 paths); found $($actual -join ', '); disallowed $($disallowed -join ', '); missing required prefixes $($missingRequiredPrefixes -join ', ')."
	}
}
